...
def register_asset(
    ...
    # V3 silent fallback 금지 원칙: enqueue 예외는 반드시 logger.warning 로 흔적
    # 을 남긴다. KeyboardInterrupt/SystemExit 같은 BaseException 은 Exception 으로
    # 잡지 않으므로 자연스럽게 re-raise 된다 (명시 분리 불필요).
    try:
        enqueuer.enqueue(saved.asset_id)
        enqueued = True
    except Exception as exc:
        logger.warning(
            "backfill enqueue failed asset_id=%d symbol=%s: %s",
            saved.asset_id,
            saved.symbol,
            exc,
        )
