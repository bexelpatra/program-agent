...
## 미션
비개발자 금융 사용자를 위한 퀀트 투자 웹앱.

**Phase 1 핵심 트랙 (백테스팅)**: **정적 자산 배분(SAA)**, **동적 자산 배분(DAA)**, **백테스팅** 세 가지이며 이 세 가지는 어떤 변경에도 항상 동작해야 한다. 아래 절대 원칙은 이 트랙에만 적용된다.

**Phase 2 관찰/탐색 트랙 (테마주 추적)**: 한국 정치 테마주 등 개별주 묶음을 사용자가 직접 큐레이션하고 정규화 차트·관심도 지표로 추적·비교. 백테스팅 트랙과는 도메인 격리 (양방향 import 금지). 절대 원칙 1·5 (JSON 미노출, MVP 프리셋 제한) 는 동일 적용되지만 2·3·4 (3요소 전략, 실거래 70%, 메트릭) 는 백테스팅 트랙 전용.
