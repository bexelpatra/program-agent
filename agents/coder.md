...
### 프런트 폼 — JSON/코드 노출 금지 원칙 (UI/UX 원칙 위반 = severity bug)
...
- MVP 시간 제약으로 임시 JSON-string 입력을 작성하지 않는다. 임시 우회는 사용자 첫 사용 사고로 직결된 사례가 있으므로 (stock-backtest 2026-04-29 weights ticker 사고). dict/array 파라미터를 만나면 **즉시 전용 위젯 신규 작성을 별도 태스크로 분리 요청**하고, 현재 태스크 report 의 "이슈/블로커" 에 명시.

### Coder 의 self-report severity 규칙 (UI/UX·실거래·보안 위반)

Coder 자신이 작성한 코드에서 아래 항목 위반을 발견하면, report 의 "이슈/블로커" 에 명시 + 해당 위반은 **observation 이 아닌 bug 로 분류**되어야 함을 Manager 에게 명시 통지한다 (severity 표기 방식이 Coder report 에는 없으므로 본문에서 "severity 권고: bug" 로 적기):

- **UI/UX 원칙 위반**: project CLAUDE.md 또는 architecture.md 의 "UI/UX 원칙" 명시 항목 (JSON 노출, 한국어 미준수, 화면 한도 초과, 접근성 미준수 등)
- **실거래/계산 정합성 원칙 위반**: look-ahead bias 의심, 정수 주 위반, FX 정밀도 손실, 비거래일 silent 0
- **보안 원칙 위반**: 인증 우회, secret 노출 (.env / api key hardcode), 입력 sanitize 누락, CORS 과도 허용

위 위반은 "기능은 동작하지만 원칙에 어긋남" 형태라 observation 으로 보고하기 쉬우나, 사용자 첫 사용 시 사고로 이어지므로 bug 로 분류해 Manager 가 후속 태스크를 강제 생성하도록 한다 (CLAUDE.md Step 4 자동 승격 규칙과 일치).
