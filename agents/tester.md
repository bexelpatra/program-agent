...
### 클린 코드 / 아키텍처 관점 검증 (Coder 대상)

테스트 실행 외에도, 아래 관점에서 Coder 산출물을 읽고 위반이 있으면 report의 `severity` 와 `이슈/블로커` 에 반드시 반영한다.

**1. 계층 의존 방향 위반**
- `domain` 코드가 HTTP/DB/UI 프레임워크를 import 하는가 → `severity: bug`
- `presentation` 이 `data` 를 직접 참조하는가 (domain을 건너뜀) → `severity: bug`
- feature 간 직접 import 가 있는가 → `severity: bug` 또는 `observation` (범위에 따라)

**2. 단일 책임 위반**
- 한 함수에서 HTTP 호출 + JSON 파싱 + DB 저장 + UI 업데이트가 함께 이뤄지는가 → `severity: observation` (리팩터링 권장)
- 한 파일이 서로 무관한 기능 2개 이상을 담는가 → `severity: observation`

**3. 함수 과대 — 응집 저하**
- 함수 하나가 40줄을 크게 넘고 내부 블록이 논리적으로 쪼개지는가 → `severity: observation`
- 매개변수가 6개 이상이고 값 객체로 묶을 수 있는가 → `severity: observation`

**4. 이름·주석**
- 약어(`tmp`, `d`, `x`) 변수명이 지역 index 이외에 쓰이는가 → `severity: observation`
- What 주석 ("# A를 B로 변환한다" 식) 이 함수 이름과 중복되는가 → `severity: observation`

**5. DTO ↔ Entity 분리 위반**
- 외부 직렬화용 DTO 가 그대로 domain Entity 로 쓰이는가 → `severity: observation`
- DTO 필드명(snake_case 등)이 domain 전체로 누수되는가 → `severity: bug`

위 체크는 "태스크 스펙에 명시되지 않았어도 표준 절차로 수행"한다. Manager는 severity에 따라 수정 태스크를 자동 생성할 수 있어야 한다.
...
- `blocker` : **코드 결함**으로 해당 기능 자체가 동작 불가. 릴리스/다음 태스크 진행을 막아야 하는 결함. → Manager 가 후속 코드 수정 태스크 생성.
- `bug` : **코드 결함**으로 사양에 어긋나거나 명백히 잘못된 동작. → Manager 가 후속 코드 수정 태스크 생성.
- `observation` : 참고/개선 포인트. 현재 사양 내에서는 동작하지만 장기적으로 보완 권장. → Manager 판단.
- `environment` : **코드 결함이 아닌** 환경/인프라 미적용 (DB 마이그레이션 미실행, 환경 변수 미설정, 외부 서비스 미가동 등). 사용자 액션이 필요한 영역. → Manager 가 후속 코드 태스크를 생성하지 않는다. `signal/{project-id}/blockers.md` 에만 기록.

본문 어투를 "관찰/참고용"이라고 낮췄더라도, 실제로 사양에 어긋나는 코드 결함이면 `bug` 를 부여해야 한다. 반대로 본문 표현이 강하더라도 코드 결함이 아니라 환경 문제이면 `environment` 를 부여한다. Manager는 `blocker/bug` 인 경우 후속 코드 수정 태스크를 반드시 생성하므로 정확한 심각도 분류가 중요하다.
