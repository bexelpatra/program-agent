...
### 7. ethics-topics (경계영역 주제 — Phase 6 신설)

사상가 중심이 아닌 **쟁점 중심** 문항을 인덱싱. `ethics-thinkers` 와 분리하여 `thinker_id` 필드 semantic 오염을 방지. exam-coverage-map.md Section C 의 경계영역 row 를 대상으로 한다.

```json
{
  "id": "string — 주제 slug (예: bioethics, environmental-ethics, information-ethics, unification-education, civic-peace)",
  "name": "string — 한글 (예: 생명의료윤리)",
  "name_en": "string — 영문",
  "category": "string — enum: applied_ethics | unification_education | civic_peace | professional_ethics | other",
  "description": "text — 주제 개요",
  "subtopics": ["string — 하위 쟁점 (예: 낙태·안락사·유전자 조작·장기이식)"],
  "key_issues": ["string — 핵심 쟁점어 / 찬반 축"],
  "related_thinker_ids": ["string — 교차 참조 사상가 id (이미 ethics-thinkers 에 존재해야 함)"],
  "related_claim_ids": ["string — 인용 가능한 ethics-claims id"],
  "exam_appearances": [
    {
      "year": "string — 예: 2017-B",
      "question_number": "string — 예: Q5",
      "summary": "string — 출제 요지"
    }
  ],
  "verbatim_sources": [
    {
...
**설계 결정**
- `related_thinker_ids` 는 ES 기존 thinker id 만 허용 (예: `singer`·`regan`·`hoffman`). 신규 생성 금지.
- `related_claim_ids` 로 `ethics-claims` 에 이미 존재하는 주장 직접 참조 (중복 저장 회피).
- `exam_appearances` / `verbatim_sources` 는 해설 작성 시 원문 grep 실증의 근거 자료.
- relations (`ethics-relations`) 스키마는 person-to-person 으로 유지. topic-to-person 교차 참조는 `ethics-topics.related_thinker_ids` 로만 한정.

**투입 대상 topic 후보** (exam-coverage-map.md Section C 기반):
| id | name | category | 예상 기출 row |
|----|------|----------|------------|
| bioethics | 생명의료윤리 | applied_ethics | 2017-B Q5 안락사 유형 등 |
| environmental-ethics | 환경윤리 | applied_ethics | 기후·동물권 일반 쟁점 |
| information-ethics | 정보윤리 | applied_ethics | AI·프라이버시 |
| professional-ethics | 직업윤리 | professional_ethics | (해당 row 확인 후 결정) |
| unification-education | 통일교육 | unification_education | 민족공동체 통일방안 등 다수 |
| civic-peace | 시민·평화 | civic_peace | 국제정치·인권규범 |
...
PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 uvicorn app:app --host 0.0.0.0 --port 8000
```

**포트 정책**: 호스트 **8000 포트는 ethics-study 전용 고정**. 다른 프로젝트(abc-english=8100, interpark_camping=8002 등)와 격리. (참고: docker container `deploy-app-1` 의 internal 8000 은 host 매핑 없음 — 충돌 무.)

### Phase A: 기출 문서 뷰어 (Track B 26개 연도 study-guide + coverage 노출)

#### 추가 요구사항
- 26개 연도 × 2 슬롯 (`{2014~2026}-{A,B}`) 의 학습 산출물을 한 라우트에서 탭 전환으로 노출:
  - `projects/ethics-study/exam-solutions/study-guide/{YYYY}-{A|B}.md` (학생용 풀이 · Track B 산출물)
  - `projects/ethics-study/exam-solutions/coverage/{YYYY}-{A|B}.md` (per-문항 심층 분석 · Phase 6)
- markdown → HTML 변환 시 **byte-level verbatim 보존** (Track B 26회 누적 규약 계승: em-dash U+2014 · 한자 CJK · ㉠~㉥ U+3220~U+3225 · 그리스어 · 독일어 결합문자 `ä·ö·ü·ß` · 프랑스어 결합문자).
- 기존 라우트 6개 (`/`, `/thinker/{id}`, `/search`, `/api/thinkers`, `/api/thinker/{id}`, `/api/search` · app.py L263·L279·L302·L322·L328·L346) **무영향 + 회귀 테스트로 보장**.

#### 추가 라이브러리
- `markdown-it-py` (CommonMark · typographic substitution OFF · byte-safe). `python-markdown` + smarty extension OFF 도 동등 — Coder 가 selection 근거를 report 에 명시.
- requirements.txt 추가 (현재 7 의존: elasticsearch / click / pyyaml / fastapi / uvicorn / jinja2 / python-multipart).

#### 신규 라우트
| 라우트 | 응답 | 비고 |
|--------|------|------|
| `GET /exam` | exam_index.html (26 연도 그리드) | study-guide·coverage 둘 다 존재하는 연도만. fs scan. |
| `GET /exam/{year}-{slot}` | exam_detail.html (탭 + TOC) | year ∈ 2014~2026 · slot ∈ {A,B} · 부재 시 404. |

#### 신규 템플릿/스태틱
- `templates/exam_index.html` — 26개 연도 카드 그리드 (연도/슬롯 link).
- `templates/exam_detail.html` — 좌측 사이드바 (연도×슬롯 nav) · 본문 (study-guide / coverage 탭 토글) · 우측 TOC (study-guide.md 만 추출, **Python 정규식 `^## 문항(?:\s+(?:서술형|논술형|기입형))?\s+\d+`** · 26개 전수 매칭 4~20 hit/file 합 293 = exam-coverage-map.md L8 "총 문항 row 수 293" 일치 · 변종 형식 `## 문항 서술형 1` (2014-B·2015-B) · `## 문항 N` (2016년 이후 통일) 둘 다 흡수). coverage.md 는 헤더 형식 분산 [`## 문항 N` · `## Q1 [...]` · `## N.` · 표 only] 로 일률 추출 불가, TOC 미적용.
- `static/style.css` 확장 — `.exam-grid`·`.exam-layout`·`.exam-tab-bar`·`.exam-tab-content`·`.exam-toc` 추가. 기존 `.tab-btn`·`.tab-count` (search.html · index.html 의 분야별 탭) 와 충돌 회피 위해 **`.exam-` prefix 강제**. 기존 1000L 변수·색상 규칙 계승.
- `static/app.js` 확장 — 탭 전환 (study/coverage) DOM display 토글 (페이지 이동 없음). 기존 `.tab-btn` click 핸들러 (app.js L13) 와 selector 분리.

#### byte-level 보존 검증 규약
- 마크다운 렌더 모듈에 `verify_verbatim(md_text: str, html_body: str) -> dict[str, tuple[int, int]]` helper.
- 검증 카운트 (HTML 텍스트 노드만 추출 후 비교):
  - em-dash `\u2014`
  - CJK 한자 `\u4E00-\u9FFF`
  - 원문자 `㉠㉡㉢㉣㉤㉥` (U+3260~U+3265 CIRCLED HANGUL · 실측 정정 from U+3220 / 양쪽 range 모두 처리 가능하나 실 데이터는 U+3260)
  - 그리스 소문자 `\u03B1-\u03C9`
  - 독일어 umlaut/eszett `[äöüÄÖÜß]`
- 각 카운트 ±0 일치. Tester 가 hexdump 로 보강 (em-dash `e2 80 94`).

#### 회귀 안전
- 기존 라우트 6개 응답 200 + body byte 변경 없음 (smoke test 추가).
- ES 연결 실패 시에도 `/exam*` 정상 동작 (마크다운 파일만 읽음 · ES 무관).
- Phase B (검색 통합) 는 기존 search route 수정이라 별도 분리.
...
- Phase 6 TASK-176 TOP10 MISS 완료 (2026-04-22): 10명 추가 (jinul·blasi·durkheim·hoffman·bandura·pettit·singer·turiel·moore·narvaez) → 65명·649 claims (ES 실측)
