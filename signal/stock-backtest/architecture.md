...
- **백테스트 universe 권고**: ETF/지수 중심 (생존편향 회피). **Phase 2 테마주 추적 모듈은 예외** — 한국 정치 테마주 등 개별주 허용 (관찰/탐색 트랙, 백테스트 universe 와 분리). 백테스트에 STOCK 자산을 universe 로 쓰면 UI 가 생존편향 경고 표시.
...
- **결정**: `assets.asset_type ∈ {EQUITY_INDEX, ETF, BOND, COMMODITY, CRYPTO, STOCK}` + `assets.market` + `assets.currency`. FX는 별도 `fx_rates` 테이블. (STOCK = 개별 상장주, Phase 2 테마 모듈에서 추가 — 백테스트 universe 사용은 생존편향 경고)
...
- `asset_type` ∈ {`EQUITY_INDEX`, `ETF`, `BOND`, `COMMODITY`, `CRYPTO`, `STOCK`, ...} — 내부 분류, 추후 UI 세분화 시 활용. `STOCK` = Phase 2 테마 모듈에서 추가된 개별 상장주 (백테스트 universe 사용 시 생존편향 경고)
...
| `allocators/{fixed_weight,all_weather,equal_weight,ma_signal}.py` | Allocator 구현 4종 (ma_signal 은 사용자 명시 추가 — TASK-219). | TASK-050, 051, 052, 219 |
...
### 6. 점진적 노출
- **Phase 1 화면 3개**: ① 백테스트 생성 (인플레이스 결과) ② 결과 다시보기 ③ 자산 카탈로그
- **Phase 2 화면 5개로 확장**: 위 3개 + ④ 테마 카탈로그 ⑤ 테마 상세/비교. 백테스팅 트랙 영향 없음 (별도 라우트 + 도메인 격리)
...
# V3 Phase 2 — 테마주 추적/관찰 모듈 (2026-05-12~ 초안)

> **상태**: 사용자 요구사항 확정 직후 초안. 정책 충돌(아래 § "기존 정책과의 충돌 — 별도 검토") 은 본 섹션 승인 후 별도 패스에서 재검토.

## 미션

뉴스·이벤트에 민감한 종목군(한국 정치 테마주 우선)을 사용자가 직접 묶고 추적·비교한다. **백테스팅 트랙과는 독립적인 관찰/탐색 트랙**이며, V3 절대 원칙 (UI 폼 우선, 한국어, 진행 가시화 등) 은 동일 적용한다.

## 핵심 결정 (사용자 확정, 2026-05-12)

| 항목 | 결정 |
|------|------|
| 1차 대상 | 한국 정치 테마주 (KOSPI/KOSDAQ 개별주) |
| 데이터 수집 | pykrx 매일 장 마감 후 (기존 KR cron 18:00 KST 잡 재활용) |
| 추가 메트릭 | **시가총액** (시계열) |
| 관심도 소스 | **네이버 데이터랩 검색량 + 구글 트렌드** (검색량 지수, 공식 API) |
| 백테스트 의존 | **0** (양방향 import 금지 — 도메인 격리) |
| 테마 변경 이력 | DB 적재 필수 (활용 방안은 Phase 2.4 이후 결정, 적재 자체는 무조건) |
| 테마 합산 차트 | 종목 합치기 + 종목 정규화(rebase=100) 비교 |
| 테마 간 비교 | 다중 테마 정규화 차트 동시 plot |

## 도메인 모델 (frozen dataclass / pydantic)

```python
# backend/app/domain/themes/entity.py
class Theme:
    theme_id: int
    name: str               # "정치 — 이재명", "정치 — 윤석열" 등 사용자 자유 입력
    slug: str               # URL-safe 식별자
    description: str | None
    user_id: str = "local"  # 단일 사용자 디폴트 (V1 결정 - architecture.md L17)
    created_at: datetime

class ThemeAsset:           # 현재 활성 멤버
    theme_id: int
    asset_id: int
    added_at: datetime
    removed_at: datetime | None  # soft delete (활성 = NULL)
    note: str | None             # 사용자 메모 (편입 사유 등)

class AssetThemeHistory:    # 자산 ↔ 테마 변경 이력 (append-only)
    history_id: int
    asset_id: int
    theme_id: int
    event_type: Literal["ADDED", "REMOVED", "RECLASSIFIED"]
    from_theme_id: int | None     # RECLASSIFIED 일 때
...
theme_assets (
    theme_id     INT FK themes ON DELETE CASCADE,
    asset_id     INT FK assets ON DELETE RESTRICT,
    added_at     TIMESTAMPTZ DEFAULT NOW(),
    removed_at   TIMESTAMPTZ NULL,           -- soft delete
    note         TEXT,
    PRIMARY KEY (theme_id, asset_id, added_at)
)
-- 인덱스: (theme_id) WHERE removed_at IS NULL  → 활성 멤버 빠른 조회

asset_theme_history (
    history_id   BIGSERIAL PK,
    asset_id     INT FK assets,
    theme_id     INT FK themes,
    event_type   VARCHAR(20) CHECK (event_type IN ('ADDED','REMOVED','RECLASSIFIED')),
    from_theme_id INT FK themes NULL,
    occurred_at  TIMESTAMPTZ DEFAULT NOW(),
    source       VARCHAR(16) DEFAULT 'USER',
    note         TEXT
)
-- append-only; trigger 또는 application layer 에서 theme_assets 변경 시 자동 기록

asset_market_cap (
    time         TIMESTAMPTZ NOT NULL,
    asset_id     INT FK assets,
    market_cap   NUMERIC(24,0),              -- 원 단위 정수
    shares_outstanding NUMERIC(20,0) NULL,   -- 상장주식수 (pykrx 별도 호출)
    PRIMARY KEY (asset_id, time)
)
-- TimescaleDB hypertable. ohlcv 와 분리 — 시가총액·발행주식수·외인순매수 등이 늘면 ohlcv 비대화 방지.

asset_attention (
    time         TIMESTAMPTZ NOT NULL,
    asset_id     INT FK assets NULL,          -- 종목 단위 점수일 때
    theme_id     INT FK themes NULL,          -- 테마 단위 점수일 때 (둘 중 하나는 NOT NULL)
    source       VARCHAR(32) NOT NULL,        -- 'naver_datalab' | 'google_trends'
    keyword      VARCHAR(120) NOT NULL,       -- 실제 검색 키워드 (감사용)
    score        NUMERIC(6,2),                -- 0~100 정규화 점수
    raw_value    NUMERIC NULL,                -- 원본 값 (있을 때)
    meta         JSONB,
    PRIMARY KEY (source, COALESCE(asset_id,0), COALESCE(theme_id,0), keyword, time),
    CHECK (asset_id IS NOT NULL OR theme_id IS NOT NULL)
)
```

### 이력 트리거 정책

`theme_assets` 의 INSERT / UPDATE(removed_at) 가 일어날 때 application layer 가 `asset_theme_history` row 를 자동 append. DB trigger 도 가능하나 SQLAlchemy/Alembic 의존성 단순화를 위해 **application layer 책임**으로 시작 (Phase 2.4 에 trigger 전환 검토).

## 데이터 수집 어댑터

### MarketCapSource (신규 Protocol)

```python
# backend/app/data/sources/base.py 또는 신규 market_cap_base.py
...
class MarketCapBar:
    time: datetime
    market_cap: int             # 원 단위 정수
    shares_outstanding: int | None
```

`PykrxMarketCapSource` 구현 — `pykrx.get_market_cap_by_date(start, end, ticker)` 호출. 기존 `PykrxSource` 와 동일 lock + 100ms rate limit 공유 (별도 lock 두지 않음).

### AttentionSource (신규 Protocol)

```python
class AttentionSource(Protocol):
    name: ClassVar[str]   # 'naver_datalab' | 'google_trends'
    def fetch_attention(
...
class AttentionBar:
    time: datetime
    score: Decimal        # 0~100 (소스별 정규화 결과 그대로 사용)
    raw_value: float | None
```

구현:
- `NaverDataLabSource`: 네이버 데이터랩 검색 API (`/datalab/search`). 클라이언트 ID/시크릿 `.env` 주입. 일일 1000 req 한도 — 백오프 + 일자 batch.
  - **TODO (Phase 2.3 시작 직전)**: `.env.example` 에 `NAVER_DATALAB_CLIENT_ID=` / `NAVER_DATALAB_CLIENT_SECRET=` 빈칸으로 선반영됨 (2026-05-12). 사용자가 developers.naver.com 에서 발급 후 채움. 미발급 상태 = SOFT BLOCKER — NaverDataLabSource 비활성, 화면 5 관심도 오버레이는 GoogleTrendsSource 만으로 graceful degradation.
- `GoogleTrendsSource`: `pytrends` 라이브러리 (비공식). 429 보호: 5초 sleep + 재시도 3회. 비공식이므로 SOFT 의존성, 실패 시 naver 만으로도 동작.

### Cron 잡 추가 (architecture.md V1 § 9 + V3 § 데이터 수집 보강)

| 잡 | 시간 (KST) | 동작 |
|----|------------|------|
| KR ohlcv | 18:00 (기존) | 변경 없음 |
| **KR 시가총액** | 18:30 (신규) | active 자산 중 `asset_type IN ('STOCK','ETF')` 대상 `PykrxMarketCapSource` 백필 |
| **관심도 (테마/자산)** | 19:00 (신규) | 활성 테마의 키워드 + theme_assets 의 종목명 키워드로 NaverDataLab 우선, GoogleTrends 보조 |

각 cron 잡은 독립 SessionLocal + 자산 단위 try/except + 3회 재시도 (V1 § 9 정책 동일). 실패는 `ingestion_log` 에 source='market_cap' / 'attention_naver' 등 구분해 기록.
...
├── __init__.py
├── entity.py              # Theme / ThemeAsset / AssetThemeHistory dataclass
├── repository.py          # ThemeRepository Protocol
├── service.py             # add/remove asset + history 기록 트랜잭션
└── normalization.py       # rebase=100 / 로그 / 누적수익률 변환 헬퍼

backend/app/domain/attention/
├── __init__.py
├── entity.py              # AttentionScore dataclass
└── aggregation.py         # 일자별 합산 / theme=종목 평균 또는 합 정책

backend/app/data/sources/
├── pykrx_market_cap_source.py
├── naver_datalab_source.py
└── google_trends_source.py

backend/app/data/repositories/
├── theme_repository.py
├── market_cap_repository.py
└── attention_repository.py

backend/app/data/
└── attention_pipeline.py  # 시가총액·관심도 백필 (ohlcv pipeline 별도)
```
...
| Method | Path | 용도 |
|--------|------|------|
| GET    | `/api/themes` | 사용자 테마 목록 (멤버 카운트 포함) |
| POST   | `/api/themes` | 테마 생성 (name 필수) |
| GET    | `/api/themes/{theme_id}` | 테마 상세 + 활성 멤버 목록 |
| PATCH  | `/api/themes/{theme_id}` | name/description 갱신 |
| DELETE | `/api/themes/{theme_id}` | 테마 soft delete (멤버 이력은 보존) |
| POST   | `/api/themes/{theme_id}/assets` | 자산 추가 (배치 가능, history 자동 append) |
| DELETE | `/api/themes/{theme_id}/assets/{asset_id}` | 자산 제거 (soft, history 자동 append) |
| GET    | `/api/themes/{theme_id}/chart?normalize=base100&weighting=equal\|market_cap&start=&end=` | 정규화 차트 데이터 (테마 내부 합산 시계열 + 멤버별 시계열) |
| GET    | `/api/themes/compare?theme_ids=1,2,3&normalize=base100&start=&end=` | 다중 테마 정규화 비교 |
| GET    | `/api/themes/{theme_id}/attention?source=naver_datalab\|google_trends&start=&end=` | 관심도 시계열 |
| GET    | `/api/assets/{asset_id}/theme_history` | 자산의 테마 이력 (RECLASSIFIED 등) |
| GET    | `/api/assets/{asset_id}/market_cap?start=&end=` | 시가총액 시계열 |

모든 응답은 V2 에러 계약 (stage/type/message/request_ctx/trace_id) 동일 적용. 페이지네이션은 `PaginatedResponse[T]` 재사용.

## 정규화 차트 사양

- **rebase=100**: 시리즈 첫 유효 가격을 100 으로 두고 비율 환산. 다른 종목 동시 plot 가능.
- **weighting**:
  - `equal`: 멤버 시리즈 산술 평균 (디폴트)
  - `market_cap`: 일자별 시가총액 가중 평균 (시가총액 데이터 누락 자산은 일시적 equal 로 폴백 + UI 경고)
- **공통 기간 처리**: universe 시작일 교집합 자동 산출 (`architecture.md` V3 § universe 시작일 불일치 정책 재사용).
- **frontend**: 기존 `EquityChart` 의 recharts 활용 + 다중 라인 + tooltip 한국어 라벨.

## Frontend 화면 추가 (UI/UX 원칙 6 갱신 — 화면 3 → 5)

| # | 경로 | 내용 |
|---|------|------|
| 1 | `/backtests/new` | 기존 (백테스트 생성 + 결과 인플레이스) |
| 2 | `/backtests/[run_id]` | 기존 (이력 진입용) |
| 3 | `/assets` | 기존 (자산 카탈로그) |
| **4** | `/themes` | **테마 카탈로그 — 목록 + 생성/편집/삭제, 멤버 카운트** |
| **5** | `/themes/[theme_id]` | **테마 상세 — 멤버 표 + 정규화 차트(가격) + 관심도 오버레이 + 합산 차트** |
| **5b** | `/themes/compare?ids=...` | **테마 간 비교 — 다중 테마 정규화 라인** (5 와 같은 라우트 트리, 별도 페이지 카운트 안 함) |

UI/UX 원칙 1~5 동일 강제. 신규 컴포넌트:
- `frontend/components/themes/{ThemeList, ThemeEditor, AssetPicker}.tsx`
- `frontend/components/themes/charts/{NormalizedPriceChart, AttentionOverlay, ThemeAggregateChart, ThemeCompareChart}.tsx`
- `frontend/hooks/useThemeChartData.ts` (다중 자산 시계열 fetch + rebase 클라이언트 변환)

## 백테스트 엔진 격리 (도메인 격리 원칙)

- `backend/app/domain/themes/` ↔ `backend/app/domain/{engine, strategy, allocators, filters}` 양방향 import 금지
- `tests/architecture/test_no_cross_import.py` (신규) 로 정적 검증 (importlib 또는 ast grep)
- 테마 모듈이 백테스트를 깨뜨릴 가능성을 구조적으로 차단 (`projects/stock-backtest/CLAUDE.md` 절대 원칙 보호)

## 기존 정책과의 충돌 — 해소 완료 (2026-05-12)

| # | 위치 | 충돌 | 결정 / 상태 |
|---|------|------|-------------|
| C1 | `architecture.md:19` | 개별 주식 제외 vs 정치 테마주 | **RESOLVED**: L19 갱신 — "백테스트 universe 권고는 ETF/지수, Phase 2 테마 모듈은 STOCK 예외 허용 + 생존편향 경고" |
| C2 | `architecture.md:93, 520` | `asset_type` 에 STOCK 누락 | **RESOLVED**: 두 위치 모두 `STOCK` 추가. alembic 마이그레이션 = 단순 Literal 확장 (기존 row 영향 없음) — Phase 2.1 첫 태스크에서 처리 |
| C3 | `projects/stock-backtest/CLAUDE.md` 미션 | "백테스팅 3가지" 만 명시 | **RESOLVED**: 미션을 Phase 1 (백테스팅) + Phase 2 (관찰/탐색) 트랙으로 분리. 절대 원칙 1·5 양 트랙 적용, 2·3·4 는 백테스팅 전용 |
| C4 | UI/UX 원칙 6 (architecture.md:740) | "MVP 화면 3개만" | **RESOLVED**: Phase 2 에서 화면 5개로 확장 명시 + 점진적 노출 원칙 유지 |
| C5 | 백테스트 universe + STOCK | 생존편향 위험 | **RESOLVED (정책)**: 백테스트 생성 화면에서 STOCK 자산 선택 시 명시 경고 토스트. UI 구현은 Phase 2.1 화면 4·5 구축 후 백테스트 화면 보강 — 별도 태스크로 분리 (Phase 2.1 후속) |

## Phase 2 내부 분할

| Phase | 범위 | 의존 |
|-------|------|------|
| **2.1** | Theme 모델 + DB 마이그레이션 + 기본 CRUD API + 화면 4 (테마 카탈로그) + 화면 5 정규화 차트 (가격만) | C1~C4 정책 결정 선행 |
| **2.2** | 시가총액 수집 (`PykrxMarketCapSource` + cron + `asset_market_cap` 백필 + market_cap_weighting) | 2.1 |
| **2.3** | 관심도 (네이버 데이터랩 우선 → 구글 트렌드 보조) + 화면 5 관심도 오버레이 | 2.1, 2.2 |
| **2.4** | 테마 변경 이력 분석 도구 (UI/통계) — 활용 방안 사용자 결정 후 | 2.1~2.3 |

## 검증 / 테스트

- **DB 무결성**: `theme_assets.removed_at` 변경 시 `asset_theme_history` row 자동 append 단위 테스트
- **정규화 수치**: rebase=100 + market_cap weighting 수학적 성질 (첫 일자 100, 일자별 비율 보존)
- **격리 정적 검증**: `tests/architecture/test_no_cross_import.py`
- **API 계약**: 기존 schemathesis fuzz 에 신규 라우트 자동 포함
- **e2e 스모크**: 테마 생성 → 자산 추가 → 차트 조회 → 자산 제거 → history 조회 (1 케이스)
- **백테스트 회귀 0**: 기존 11/11 validation harness + 9 골든 baseline 영향 없음을 마이그레이션 후 재확인

## 알려진 제약 / 향후

- 네이버 데이터랩 API 의 일일 호출 한도 (1000) — 테마/종목 수가 늘면 batch 우선순위 큐 필요 (Phase 2.3 결정)
- pytrends 비공식 의존성 — 차단 시 naver 만으로도 화면 5 동작해야 함 (graceful degradation)
- 시가총액 weighting 사용 시 일부 자산 누락 → equal fallback 명시 UI 통지
- 테마 변경 이력 활용 방안 미정 (Phase 2.4 사용자 결정 후 분석 도구화)
